defmodule Phoenix.PubSubTest do
  ...
  setup config do
    ...
    {:ok, _} =
      Phoenix.PubSub.ConduitAMQP.start_link(config.test, pool_size: size, broker: TestBroker)
    start_supervised({TestBroker, []})
  ...
  for size <- [1, 8] do
    ...
    test "pool #{size}: subscribe/3 with link downs subscriber", config do
      ...
      assert rpc(non_linked_pid2, fn ->
               PubSub.subscribe(config.test, config.topic, link: false)
             end)
